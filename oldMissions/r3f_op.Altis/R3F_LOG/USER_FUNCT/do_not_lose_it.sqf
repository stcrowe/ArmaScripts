...
if (!isNull _object) then
{
	...
	if (R3F_LOG_active) then
	{
		...
		if (isServer) then
		{
			...
			R3F_LOG_liste_objects_a_proteger pushBack _object;

			R3F_LOG_mutex_local_lock = false;

			if (_pos_respawn == "spawn_pos" || _pos_respawn == "exact_spawn_pos") then
			{
				_object setVariable ["R3F_LOG_pos_respawn", getPosASL _object, false];

				if (_pos_respawn == "exact_spawn_pos") then
				{
					_object setVariable ["R3F_LOG_dir_respawn", getDir _object, false];
				};
			}
			else
			{
				_object setVariable ["R3F_LOG_pos_respawn", _pos_respawn, false];
			};
		};
	};
};